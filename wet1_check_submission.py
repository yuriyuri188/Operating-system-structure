...
ALLOWED_O_FILES = ['my_system_call.o', 'my_system_call_c.o']
...
def check_folder_contents(contents, required_files_case_insensitive, allowed_file_types):
    ...
    for filename in contents:
        ...
        elif filename in ALLOWED_O_FILES:
            print('Found allowed object file:', filename)
        else:
            found_errors = True
...
def build(path, exec_file):
    ...
    extra_o = 'my_system_call.o' if cpp else 'my_system_call_c.o'
    if extra_o in ALLOWED_O_FILES:
        source_files = source_files + [extra_o]
